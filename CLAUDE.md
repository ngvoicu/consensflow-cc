...
- **Single shared home:** the participant roster is `~/.consensflow/participants.json` (`participantsPath()` → `configHome()`), so a participant defined in either tool is available in the other. There are no per-tool config roots; run artifacts and session stash live under `~/.consensflow/workspaces/…` (`configRoot()` = `configHome()`).
