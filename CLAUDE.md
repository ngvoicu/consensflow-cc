...
- **Participants are read-only by default**: `effectiveToolsPolicy` treats a missing tools policy as `readonly`; only an explicit `workspace-write`/`full-auto` policy makes one write-capable. Enforcement is per engine in runners.js (codex sandbox, claude allow+deny lists, pi tools allowlist, `OPENCODE_PERMISSION` env).
