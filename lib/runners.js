...
export function buildRunnerInvocation(participant, packetPath, cwd) {
  ...
  switch (p.kind) {
    ...
    case "claude-code": {
      ...
      // UserPromptSubmit hook could stomp the lead session's .consensflow-cc/session.json stash.
      const args = ["-p", "Follow the ConsensFlow packet provided on stdin. Return only the requested output.", "--output-format", "json", "--no-session-persistence", "--bare", "--allowedTools", claudeAllowedTools(p.toolsPolicy)];
