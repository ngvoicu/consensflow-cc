...
export const PARTICIPANT_PRESETS = [
  ...
  // --- Image generation (Codex backend → gpt-image-2) ---------------------
  {
    ...
    description: "Generates images with gpt-image-2 through your existing Codex (ChatGPT) login. The model field is only the trigger model; the image backend is always gpt-image-2.",
    kind: "image",
    model: "gpt-5.5",
    roles: ["reviewer"],
    toolsPolicy: "readonly",
...
export function formatPresets(cli = "cf") {
  return ["# ConsensFlow participant presets", "", ...PARTICIPANT_PRESETS.map(formatPresetLine), "", `Add one with \`${cli} participants add <preset>\` (or \`add all\`) — or just ask in plain words ("add the zeus preset").`].join("\n");
}
