...
- **Streaming observability (primary):** `cf run @name --stream` renders normalized thinking / tool-call / answer events (via `renderEvent`) to stdout as they arrive — **foreground-incremental**, so the lead relays them into the session. Hook-routed participant calls include `--stream` and should run foreground unless the user explicitly asks otherwise. Without `--stream`, the run prints just the clean final answer. Every text-CLI run also writes a human-readable `transcript.md` (the event trail) into its run dir as a **durability backstop** — the record that survives an interrupted run — and `result.transcriptPath` points to it. On timeout/no-answer the output is the bounded trail under a clear header, never the raw JSONL stream and never a bare whitespace fragment. `--stream`/`--rw`/`--tools`/`--json` are flags-after-prompt (parseOptions consumes a flag's next token).
