...
- `lib/*.js` — plain JS, the unit-tested core:
  ...
  - `workflows.js` — `runNamedParticipant`. Verbatim from pi (parity-locked).
...
- Participants run as standard read-write CLI calls — exactly like running claude/codex/pi/opencode yourself: they read, edit files, and run commands, confined to the project workspace by each engine's own defaults. There is no permission knob (removed 2026-08-20) and no bypass flag is ever passed. Claude/codex children get `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` stripped (subscription-login billing guard).
