...
- **Streaming observability (primary):** `cf run @name --stream` renders normalized thinking / tool-call / answer events (via `renderEvent`) to stdout as they arrive — **foreground-incremental**, so the lead relays them into the session. Hook-routed and `/consensflow:cf` participant calls always include `--stream` and must stay in the foreground (non-optional): the lead must not drop `--stream`, detach the run, or summarize the streamed trail away — the only exception is an explicit user request for `--json`. The parsed final answer is always printed again after the child exits, even with `--stream`, so streamed runs end with a durable reply section. Without `--stream`, the run prints just the clean final answer. Every text-CLI run also writes a human-readable `transcript.md` (the event trail) into its run dir as a **durability backstop** — the record that survives an interrupted run — and `result.transcriptPath` points to it. On timeout/no-answer the output is the bounded trail under a clear header, never the raw JSONL stream and never a bare whitespace fragment. Run flags are schema-aware and may appear before or after the prompt; unknown `--flags` are preserved as prompt text.
- **Per-call tools override:** participants are already write-capable by default (`workspace-write`), so `--rw` is now redundant (it just equals the default). `--tools full-auto` is the only escalation — it bypasses the engine's sandbox/approval checks for one run without a second roster entry. The consent gate is unchanged.
